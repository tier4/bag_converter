...
- `t_us` (uint32): Relative timestamp in microseconds from the scan start time (xyzit, en_xyzit only)
...
For backward compatibility, `t_us` is also available and can be calculated as `timestamp / 1000`.
